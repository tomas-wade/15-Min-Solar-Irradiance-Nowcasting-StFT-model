'''
- predicts 15 min intervals for hour ahead for several locations
'''
...
class PatchEncoder(nn.Module):
    def __init__(self, in_channels=2, hidden=64, out_dim=128):
        ...
        self.conv = nn.Sequential(
            ...
            nn.ReLU(),
            nn.GroupNorm(8,64), #group norm instead of batch norm - doesn't depend on batch statistics, steadier across the more varied multi-site batches this version handles
        )
...
#the full StFT fusion model (version 8 / "general", multi-site): same fusion architecture as the single-site model, but
#larger (D=192 vs 128, 3 fusion layers vs 2) and additionally conditioned on each sample's lat/lon so one trained model
#can generalise across several different stations instead of being implicitly tied to a single location
class StFTFusionModelGeneral(nn.Module):
    """
    ...
    def __init__(
        self,
        D=192,
        spec_dim=96,            # interpreted as per-channel spectral dim
        ...
        self.init_time_proj = nn.Linear(5, D) #projects [sin(hour), cos(hour), solar_altitude_norm, lat_norm, lon_norm] into a D-dim embedding - 2 dims more than the single-site model, for lat/lon conditioning
    ...
    def forward(self, batch):
        ...
        # build lat & lon arrays from meta (fall back to model.center_lat/lon)
        #this lat/lon conditioning block is what's new in v8 - it's what lets one trained model generalise across multiple sites
        lat_list = []
        lon_list = []
        if isinstance(meta, list) and len(meta) == B and isinstance(meta[0], dict):
            for m in meta:
                lat = m.get('center_lat', None)
                lon = m.get('center_lon', None)
                if lat is None: lat = self.center_lat
                if lon is None: lon = self.center_lon
                lat_list.append(float(lat) if lat is not None else 0.0)
                lon_list.append(float(lon) if lon is not None else 0.0)
        else:
            lat_list = [self.center_lat if self.center_lat is not None else 0.0] * B
            lon_list = [self.center_lon if self.center_lon is not None else 0.0] * B

        lat_t = torch.tensor(lat_list, dtype=torch.float32, device=device).unsqueeze(-1)
        lon_t = torch.tensor(lon_list, dtype=torch.float32, device=device).unsqueeze(-1)

        # normalize lat/lon to roughly [-1,1]
        lat_norm = lat_t / 90.0 #90 degrees is the max possible latitude
        lon_norm = lon_t / 180.0 #180 degrees is the max possible longitude

        # final t_input: sin,cos,solar_norm,lat_norm,lon_norm -> (B,5)
        t_input = torch.cat([t_sincos.to(device).float(), solar_norm.to(device).float(), lat_norm, lon_norm], dim=1)
